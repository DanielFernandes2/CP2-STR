...
void Task1 (void *pv)
{
    ...
    for(;;)
    {
        ...
        if(var == NULL)
        {
            printf("{Daniel Fernandes-RM86936} Não foi possível alocar dinamicamente\n");
        ...
        {
            printf("{Daniel Fernandes-RM86936} Valor não enviado\n");
            free(var);
        }
        else
        {
            ...
            printf("{Daniel Fernandes-RM86936} Valor %d de ID %d enviado com sucesso\n", var->random_number, var->id);
        }
...
void Task2(void *pv)
{
    ...
    for(;;)
    {
        ...
        {
            printf("{Daniel Fernandes-RM86936} Valor %d de ID %d recebido com sucesso\n", pointer->random_number, pointer->id);
        ...
        {
            timeout++;
            printf("{Daniel Fernandes-RM86936} Não foi possível receber valor\n");
        }
 
        if(timeout == 5)
        {
            printf("{Daniel Fernandes-RM86936} Não foi possível receber valor da fila\n");
        }
        else if(timeout == 10)
        {
            printf("{Daniel Fernandes-RM86936} Recuperação moderada - Limpa fila\n");
            xQueueReset(fila);
        }
        else if (timeout == 15)
        {
            printf("{Daniel Fernandes-RM86936} Recuperação agressiva - Resetar o sistema\n");
            vTaskDelay(pdMS_TO_TICKS(1000));
            esp_restart();
        }
...
void Task3 (void *pv)
{
    for(;;)
    {
        ...
        {
            printf("{Daniel Fernandes-RM86936} Sistema OK (Task1 e Task2 ativas)\n");
        }
        else if(bits & BIT_TASK1_OK)
        {
            printf("{Daniel Fernandes-RM86936} Sistema parcialmente OK (apenas Task1)\n");
        }
        else if(bits & BIT_TASK2_OK)
        {
            printf("{Daniel Fernandes-RM86936} Sistema parcialmente OK (apenas Task2)\n");
        }
        else
        {
            printf("{Daniel Fernandes-RM86936} Sistema com falha (nenhuma task sinalizou)\n");
        }
        flag = 0;
...
void app_main(void)
{
    ...
    {
        printf("{Daniel Fernandes-RM86936} Falha na criação da fila ou do event group\n");
        esp_restart();
    }
